...
typedef NS_ENUM(NSUInteger,ZoomVideoSDKERROR)
{
    ...
    Errors_Session_Dont_Support_Feature,
...
typedef NS_ENUM(NSUInteger, ZoomVideoSDKPhoneFailedReason) {
    ...
    ZoomVideoSDKPhoneFailedReason_Block_No_Host,///<Disable the function of international call-out before the host joins the meeting.
    ZoomVideoSDKPhoneFailedReason_Block_High_Rate,///<The call-out is blocked by the system due to the high cost.
    ZoomVideoSDKPhoneFailedReason_Block_Too_Frequent,///<All the invitees invited by the call should press the button one(1) to join the meeting. In case that many invitees do not press the button that leads to time out, the call invitation for this meeting shall be banned.
};
